...
if __name__=="__main__":
    ...
    if use_demo:
        ...
        raw_images_dir = './0_raw_data/raw_imgs/'  # 原始图片保存位置
        raw_ann_dir = './1_raw2voc/all/'  # voc格式的标注文件保存位置
        slice_voc_dir = './2_make_slice_voc/all/labels/'  # 切图后标注文件保存位置
        outdir = './2_make_slice_voc/all/imgs/'  # 切图后图像保存位置
        if not os.path.exists(slice_voc_dir): os.makedirs(slice_voc_dir)
        if not os.path.exists(outdir): os.makedirs(outdir)
        cnt=0
