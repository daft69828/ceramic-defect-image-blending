...
# 1、生成图片保存的路径
CKimg_dir = './dataset_voc_for_mmdetection/test/voc/images/'
# 2、生成标注文件保存的路径
CKanno_dir = './dataset_voc_for_mmdetection/test/voc/annotations/'
...
def get_CK5(origin_anno_dir, origin_image_dir, verbose=False):
    dataTypes = ['test']       # 3、根据你要转化的是训练集还是验证集（annotations名称）
...
def main():
    base_dir = './dataset_voc_for_mmdetection/test/voc/'  # step1 这里是一个新的文件夹，存放转换后的图片和标注
    image_dir = os.path.join(base_dir, 'images')  # 在上述文件夹中生成images，annotations两个子文件夹
    anno_dir = os.path.join(base_dir, 'annotations')
    mkr(image_dir)
    mkr(anno_dir)
    # 4、
    origin_image_dir = './coco/test/images/'  # step 2原始的coco的图像存放位置
    # 5、
    origin_anno_dir = './coco/test/annotations/'  # step 3 原始的coco的标注存放位置
