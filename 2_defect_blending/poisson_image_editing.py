...
def main():    
    scr_dir = './results'
    out_dir = scr_dir
    source = cv2.imread(path.join(scr_dir, "res_img.png"))
    target = cv2.imread(path.join(scr_dir, "target.jpg"))
    mask = cv2.imread(path.join(scr_dir, "res_msk.png"), cv2.IMREAD_GRAYSCALE)
    source = source[900:1100, 900:1100]
    target = target[900:1100, 900:1100]
    mask = mask[900:1100, 900:1100]

    offset = (0, 0)
    result = poisson_edit(source, target, mask, offset)

    cv2.imwrite(path.join(out_dir, "possion.png"), result)
